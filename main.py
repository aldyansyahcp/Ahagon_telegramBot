    ...
    try:
        ...
        if tek > len(rez):
            ...
            for i in isi.find_all("a", attrs={"data-wpel-link":"external"}):
                ...
                key.row(types.InlineKeyboardButton(text="Open link",url=link))
                bot.send_message(cet,i.string,reply_markup=key)
                #key = types.KeyboardButton(n)
                #markup.add(key)
                #lin.append(link)
                #n+=1
...
def webhook():
    try:
        ...
        res = "<h1><b>BOT STARTED</b></h1>", 200
    except Exception as e:
        res = "<h1>False</h1><br>{}".format(e)
    return res
