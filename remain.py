...
#apik = "5131229412:AAEMk2nkxlkG_ZoY9vG91E8ljcmusQqTCwA"
apik = "5296878013:AAFvVyUdUBGty1vNaj1LNGqU4N4P_EHpzNw"
#api = "==wdOpHcIV0XQRjT0UVcH5ETxoWYOZXM5R3RCVFZVlnV2ZUQBpzMxADO3gjN5ITN"[::-1].encode("ascii")
...
def main(msg):
    ...
    #na = bot.send_photo(cet, open("beliau.jpg", "rb"))
    #lin = requests.get("https://acefile.co/player/79391298")
    #bot.send_video(cet, text='<iframe src="https://acefile.co/player/79391298" FRAMEBORDER=0 MARGINWIDTH=0 MARGINHEIGHT=0 SCROLLING=NO WIDTH="640" HEIGHT="350" allowfullscreen="true"></iframe>', parse_mode="HTML")
...
def three(msg,res):
    try:
        ...
        cek = []
        n=1
        req = ses.get(res[int(c)-1],headers=headers)
        ...
        resj += "\n\nPilih Download Batch,Movie atau Perepisode"
        na = bot.send_message(cet, resj, reply_markup=markup)
        bot.register_next_step_handler(na,four, res[int(c)-1])
        global dould
        dould = res[0]
    except Exception as ex:
...
def four(msg,fin):
    try:
        ...
        if tek == "Batch":
            for i in name:
                ...
                if "-batch-" in lin or "batch" in lin:
                    ...
                    #bot.send_photo(cet, photo=open("pict/beliau.jpg","rb"))
                    pass
        ...
        if len(resj) > 4500:
            ...
            na = bot.send_message(cet, resj, reply_markup=markup)
            bot.register_next_step_handler(na,five,resdu)
    except Exception as ex:
    ...
    try:
        ...
        if tek > len(rek):
            ...
            key.add(
                ...
                types.InlineKeyboardButton(text="Kembalieps", callback_data="backeps")
            )
    ...
    try:
        ...
        if tek > len(rez):
            ...
            key.add(
                types.InlineKeyboardButton(text="Done", callback_data="done"),
                types.InlineKeyboardButton(text="Again", callback_data="again"),
                types.InlineKeyboardButton(text="Kembali eps", callback_data="backeps"),
            )
            bot.send_message(cet,"Here Your download link",reply_markup=key)
                #key = types.KeyboardButton(n)
                #markup.add(key)
...
def kolbek(msg):
    if msg.message:
        ...
        elif msg.data == "backeps":
            four(msg.message, dould)
        else:
            pass
